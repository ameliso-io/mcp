...
export function useCaseEdit({ repoId, setCases, setError, lastFocusRef, editingBodyRef, editingPath, setEditingPath, announceAction }: Params) {
  ...
  async function startEdit(c: Case) {
    ...
    setEditTitle(c.title); setEditDesc(c.description); setEditPriority(stringToPriority(c.priority));
    setEditTags(c.tags.join(", ")); setEditBody(""); setEditNewPath("");
  ...
  async function handleUpdate(e: React.FormEvent) {
    ...
    try {
      const updated = await client.updateCase({
        repoId, casePath: editingPath, title: editTitle, description: editDesc, priority: editPriority,
        tags: editTags ? editTags.split(",").map((t) => t.trim()).filter(Boolean) : [],
        body: editBody, newPath: editNewPath,
      });
      if (updated.case) {
  ...
  return { editTitle, setEditTitle, editDesc, setEditDesc, editPriority, setEditPriority, editTags, setEditTags, editBody, setEditBody, editNewPath, setEditNewPath, saving, startEdit, handleUpdate };
}
