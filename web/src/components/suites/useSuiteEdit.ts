...
export function useSuiteEdit({ repoId, load, setError, lastFocusRef, announce }: Params) {
  ...
  async function handleUpdate(e: React.FormEvent) {
    ...
    try {
      await client.updateSuite({
        repoId, slug: editingSlug, name: editName, description: editDesc,
        cases: editCases ? editCases.split(",").map((c) => c.trim()).filter(Boolean) : [],
        replaceCases: true, newSlug: editNewSlug,
      });
      setEditingSlug(null);
  ...
  return { editingSlug, setEditingSlug, editName, setEditName, editDesc, setEditDesc, editCases, setEditCases, editNewSlug, setEditNewSlug, saving, startEdit, handleUpdate };
}
