...
export function useSuitesList({ repoId, announceFilter }: Params) {
  ...
  const filteredSuites = q ? suites.filter((s) => s.name.toLowerCase().includes(q) || s.slug.toLowerCase().includes(q) || s.description.toLowerCase().includes(q)) : suites;

  useEffect(() => {
    if (loading || !q) return;
    const count = filteredSuites.length;
    if (prevFilterCountRef.current !== null && prevFilterCountRef.current !== count) announceFilter(count === 1 ? "1 suite found" : `${count} suites found`);
    prevFilterCountRef.current = count;
  }, [filteredSuites.length, loading, q, announceFilter]);

  return { suites, setSuites, loading, error, setError, load, search, setSearch, q, filteredSuites };
}
