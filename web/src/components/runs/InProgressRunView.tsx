    ...
    <>
      {totalInScope > 0 && (
        <div className={styles.progressWrap}>
          ...
          <div className={styles.progressTrack} role="progressbar" aria-label="Run completion progress" aria-valuemin={0} aria-valuemax={totalInScope} aria-valuenow={totalInScope - pendingCases.length} aria-valuetext={`${totalInScope - pendingCases.length} of ${totalInScope} case${totalInScope !== 1 ? "s" : ""} complete`}>
            <div className={styles.progressBar} style={{ width: `${((totalInScope - pendingCases.length) / totalInScope) * 100}%` }} />
          </div>
        </div>
      )}
      <PendingHeader runId={runId} />
      {pendingCases.length === 0 && <p className={styles.allDone}>All cases have results recorded.</p>}
        ...
        <div className={styles.recordedSection}>
          ...
          <ul className={styles.resultList} role="list">
            {recordedResults.map((r) => (
              <li key={r.casePath} className={styles.resultRow} aria-label={`${statusLabel(r.status)}: ${r.casePath}`}>
                <span className={styles.resultStatusBadge} data-status={ResultStatus[r.status]}>{statusLabel(r.status)}</span>
                {caseTitleMap.get(r.casePath)?.title && <span className={styles.resultTitle}>{caseTitleMap.get(r.casePath)?.title}</span>}
      ...
      <ul className={styles.pendingList} role="list">
        {pendingCases.map((c) => <PendingCaseRow key={c.path} c={c} />)}
      </ul>
    </>
  );
}
