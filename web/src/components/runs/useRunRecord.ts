...
export function useRunRecord({
  ...
  const handleRecord = useCallback(
    async (e: React.FormEvent) => {
      ...
      setRecording(true);
      try {
        await client.recordResult({ repoId, runId: selectedRunId, casePath: recordingCase, status: recordStatus, notes: recordNotes });
    ...
    [selectedRunId, recordingCase, repoId, recordStatus, recordNotes, announce, setRecordingCase, setCaseBody, setPendingCases, setTotalInScope, setRecordedResults, setError, lastFocusRef]
  );
  ...
  return { recordStatus, setRecordStatus, recordNotes, setRecordNotes, recording, caseBodyLoading, handleRecord, openRecord };
}
