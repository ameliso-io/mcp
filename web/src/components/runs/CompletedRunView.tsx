...
export default function CompletedRunView() {
  const { recordedResults, deferredFilteredResults, isResultsStale, caseTitleMap } = useRunsTabContext();
  ...
  return (
    <div>
      ...
      {recordedResults.length === 0 ? (
        ...
        <ul
          className={isResultsStale ? `${styles.resultList} ${styles.resultListStale}` : styles.resultList}
          role="list"
