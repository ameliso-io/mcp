        ...
        {confirmingFinalize?.runId === runId ? (
          <>
            <span className={styles.confirmText}>
              {confirmingFinalize.status === RunStatus.COMPLETED ? "Complete?" : "Abort?"}
            </span>
            <button
              type="button"
              aria-label={`Confirm ${confirmingFinalize.status === RunStatus.COMPLETED ? "complete" : "abort"} run ${runId}`}
              onClick={() => handleFinalize(runId, confirmingFinalize.status)}
              className={
          ...
          <>
            <button
              type="button"
              aria-label={`Complete run ${runId}`}
              onClick={() => {
                setConfirmingFinalize({ runId, status: RunStatus.COMPLETED });
              }}
            ...
            <button
              type="button"
              aria-label={`Abort run ${runId}`}
              onClick={() => {
                setConfirmingFinalize({ runId, status: RunStatus.ABORTED });
              }}
