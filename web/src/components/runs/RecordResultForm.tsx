...
export default function RecordResultForm({ casePath }: { casePath: string }) {
  const { recordStatus, setRecordStatus, recordNotes, setRecordNotes, recording, caseBodyLoading, caseBody, handleRecord, setRecordingCase, lastFocusRef } =
    useRunsTabContext();
      ...
      >
        <div>
          <label className={styles.labelSm}>
            Status
            <select value={recordStatus} onChange={(e) => { setRecordStatus(Number(e.target.value)); }} autoFocus className={styles.inputAuto}>
        ...
        <div className={styles.notesWrap}>
          <label className={recordStatus === ResultStatus.FAILED || recordStatus === ResultStatus.BLOCKED ? styles.labelSmErr : styles.labelSm}>
            ...
            <input
              ...
              required={recordStatus === ResultStatus.FAILED || recordStatus === ResultStatus.BLOCKED}
              aria-required={recordStatus === ResultStatus.FAILED || recordStatus === ResultStatus.BLOCKED}
              maxLength={2000}
              className={recordStatus === ResultStatus.FAILED || recordStatus === ResultStatus.BLOCKED ? styles.inputErr : styles.input}
            />
          </label>
        </div>
