...
describe('RepositoriesTab', () => {
  ...
  it('shows repo card with name and badges', async () => {
    ...
    expect(screen.getByText('/clones/repo')).toBeInTheDocument()
  })

  it('shows Active badge for active repo', async () => {
    vi.mocked(client.listRepositories).mockResolvedValue({ repositories: [makeRepo()] } as never)
    render(<RepositoriesTab onRepoSelect={() => {}} activeRepoPath="/clones/repo" />)
    await waitFor(() => expect(screen.getByText('Active')).toBeInTheDocument())
  })

  it('calls onRepoSelect when Use clicked', async () => {
    vi.mocked(client.listRepositories).mockResolvedValue({ repositories: [makeRepo()] } as never)
    const onRepoSelect = vi.fn()
    render(<RepositoriesTab onRepoSelect={onRepoSelect} activeRepoPath="" />)
    await waitFor(() => screen.getByText('Use'))
    await userEvent.click(screen.getByText('Use'))
    expect(onRepoSelect).toHaveBeenCalledWith('/clones/repo')
  })

  it('calls onRepoSelect with empty string when Deselect clicked', async () => {
    vi.mocked(client.listRepositories).mockResolvedValue({ repositories: [makeRepo()] } as never)
    const onRepoSelect = vi.fn()
    render(<RepositoriesTab onRepoSelect={onRepoSelect} activeRepoPath="/clones/repo" />)
  ...
  it('shows Clone button for repo not yet cloned', async () => {
    vi.mocked(client.listRepositories).mockResolvedValue({ repositories: [makeRepo({ cloned: false })] } as never)
    render(<RepositoriesTab onRepoSelect={() => {}} activeRepoPath="" />)
    await waitFor(() => expect(screen.getByText('Clone')).toBeInTheDocument())
  ...
  it('calls handleGitHubCallback when installation_id present without setup_action', async () => {
    ...
    vi.mocked(client.handleGitHubCallback).mockResolvedValue({ repositories: [makeRepo()] } as never)
    render(<RepositoriesTab onRepoSelect={() => {}} activeRepoPath="" />)
    await waitFor(() => expect(client.handleGitHubCallback).toHaveBeenCalledWith({ installationId: 'inst-no-action' }))
    window.history.replaceState({}, '', '/')
  })
  ...
  it('updates repo in list when syncRepository succeeds with repository', async () => {
    ...
    vi.mocked(client.listRepositories).mockResolvedValue({ repositories: [makeRepo(), otherRepo] } as never)
