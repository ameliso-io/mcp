...
describe("RunsTab", () => {
  ...
  it("shows error when finalizeRun fails", async () => {
    vi.mocked(client.listRuns).mockResolvedValue({ runs: [mockRun] } as never);
    vi.mocked(client.finalizeRun).mockRejectedValue(new Error("finalize error"));
    vi.spyOn(window, "confirm").mockReturnValue(true);
    render(<RunsTab repoId="owner/repo" />);
    await waitFor(() => screen.getByText("2026-01-01-smoke"));
    await userEvent.click(screen.getByText("2026-01-01-smoke"));
    await waitFor(() => screen.getByText("Complete Run"));
    await userEvent.click(screen.getByText("Complete Run"));
    await waitFor(() => expect(screen.getByText("finalize error")).toBeInTheDocument());
  });

  it("polling timer callback refreshes pending cases", async () => {
    vi.mocked(client.listRuns).mockResolvedValue({ runs: [mockRun] } as never);
    vi.mocked(client.getPendingCases).mockResolvedValue({
      cases: [mockCase],
      totalInScope: 1,
