...
| Directory | Purpose |
|-----------|---------|
| `server/` | gRPC server (tonic 0.12). Implements `AmelisoService` (16 RPCs). |
| `mcp/` | MCP server (rmcp 1.5, stdio). Wraps all 16 RPCs as MCP tools. |
| `cli/` | CLI binary (clap 4). Wraps all 16 RPCs as subcommands. |
